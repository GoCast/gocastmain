...
AndroidSurfaceViewRenderer::Init()
{
	...
	jclass javaRenderClassLocal = env->FindClass("com/gocast/kddidemo/ViESurfaceRenderer");
        ...
        const float bottom)
{
    ...
    jclass javaRenderClass = env->FindClass("com/gocast/kddidemo/ViESurfaceRenderer");
    if (!javaRenderClass)
    {
        WEBRTC_TRACE(kTraceError, kTraceVideoRenderer, _id,  "%s: could not find ViESurfaceRenderer", __FUNCTION__);
        return -1;
