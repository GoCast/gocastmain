...
function Signin() {
  ...
  var serverIp = $("#ip").val();
  var serverPort = parseInt( $("#port").val() );
...
function LoadCallPage() {
  ...
  for(i=0; i<OnlinePeers.length; i++) {
    usersOnCall = OnCallWithPeer[OnlinePeers[i]];

    if(false == OnCallWithPeer[OnlinePeers[i]])  {
      offlinePeers += renderTemplate("user-template", { username: OnlinePeers[i], status: "offline", action: "Add to video",    caption: "Available", avatar: getGravatar(OnlinePeers[i]) })
    } else {
      onlinePeers  += renderTemplate("user-template", { username: OnlinePeers[i], status: "online",  action: "Drop from video", caption: "On Call"  , avatar: getGravatar(OnlinePeers[i]) })
    }
  }

  if (usersOnCall) {
    $("#video-panel").addClass("running")
  } else {
    $("#video-panel").removeClass("running")
  }

  if (offlinePeers != "") {
    $("#available-users").html( offlinePeers );
  } else {
    $("#available-users").html( renderTemplate("no-peers", { message: "No available peers connected" }) )
  }

  if (onlinePeers != "") {
    $("#on-call-users").html( onlinePeers )
  } else {
    $("#on-call-users").html( renderTemplate("no-peers", { message: "No peers on call" }) )
  }
}
...
function LoadSigninPage() {
  $("#peers-content").hide();
  $("#not_installed").remove();
  $sign_in = $("#sign_in_panel")
  $sign_in.fadeIn(1000);
  $sign_in.find("form").submit(function()  {
    Signin();
    $sign_in.hide();
    $("#peers-content").show();
    return false;
  ...
  $(".video-call").live("click", function(){
    peername = $(this).attr("data-peername")
    CallOrHangup(peername)
  });
...
setTimeout('$("#not_installed").fadeIn("slow")', 2000);
...
function renderTemplate(template, context) {
  ...
  return Mustache.to_html(templateHtml, context);
}
...
function getGravatar(email) {
  ...
  return 'http://www.gravatar.com/avatar/' + MD5(email) + '.jpg?s=50'
}
