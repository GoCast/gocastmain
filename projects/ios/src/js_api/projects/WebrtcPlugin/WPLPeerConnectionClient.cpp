...
static void refreshPeerList(Peers& p)
{
    if (gInstance)
    {
        [gInstance clearPeerList];
        Peers::iterator iter;
        
        iter = p.begin();
        while (iter != p.end())
        {
            [gInstance appendPeerListWithString:(*iter).second];
            iter++;
        }
        [gInstance updatePeerList];
    }
}
#endif
