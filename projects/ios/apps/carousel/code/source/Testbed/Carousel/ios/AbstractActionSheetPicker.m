...
#pragma unused(target, successAction, origin)
    NSAssert(NO, @"This is an abstract class, you must use a subclass of AbstractActionSheetPicker (like ActionSheetStringPicker)");
}
...
- (IBAction)customButtonPressed:(id)sender {
    ...
    NSAssert((index >= 0 && index < (int)self.customButtons.count), @"Bad custom button tag: %d, custom button count: %d", index, self.customButtons.count);
    ...
    NSDictionary *buttonDetails = [self.customButtons objectAtIndex:(unsigned int)index];
