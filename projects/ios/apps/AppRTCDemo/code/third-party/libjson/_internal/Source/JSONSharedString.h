...
public:
	...
	inline json_string::iterator std_begin(void){
		return _str -> mystring.begin() + (int)offset;
	}
	inline json_string::iterator std_end(void){
		return std_begin() + (int)len;
	}
	...
	inline json_shared_string(const iterator & s, const iterator & e) : _str(s.parent -> _str), offset(size_t(s.it - s.parent -> _str -> mystring.data())), len(size_t(e.it - s.it)){
		++_str -> refCount;
	}
	...
	size_t find(json_char ch, size_t pos = 0) const {
		...
		for(json_string::const_iterator b = std_begin() + (int)pos; b != e; ++b){
			if (*b == ch) return size_t(b - std_begin());
		}
