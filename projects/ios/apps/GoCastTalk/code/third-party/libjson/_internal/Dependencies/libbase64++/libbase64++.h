...
namespace libbase64 {
	...
	static STRINGTYPE encode(const unsigned char * binary, size_t bytes){
		...
		for (size_t i = 0, aligned((bytes - misaligned) / 3); i < aligned; ++i){
			...
			result += libbase64_characters::getChar<CHARTYPE>(uint8_t(((GETITEM_BOUNDCHECK(binary, binarybounds) & 0x03) << 4) + ((GETITEM_BOUNDCHECK(binary + 1, binarybounds) & 0xF0) >> 4)));
			result += libbase64_characters::getChar<CHARTYPE>(uint8_t(((GETITEM_BOUNDCHECK(binary + 1, binarybounds) & 0x0F) << 2) + ((GETITEM_BOUNDCHECK(binary + 2, binarybounds) & 0xC0) >> 6)));
		...
		if (libbase64_likely(misaligned != 0)){
			...
			result += libbase64_characters::getChar<CHARTYPE>(uint8_t(((temp[0] & 0x03) << 4) + ((temp[1] & 0xF0) >> 4)));
			if (misaligned == 2){
				LIBBASE64CODECOVERAGEBRANCH;
				result += libbase64_characters::getChar<CHARTYPE>(uint8_t(((temp[1] & 0x0F) << 2) + ((temp[2] & 0xC0) >> 6)));
			} else {
				LIBBASE64CODECOVERAGEBRANCH;
				result += (CHARTYPE)'=';
			}
    ...
    static std::string decode(const STRINGTYPE & encoded){
		...
		for (unsigned int i = 0; i < aligned; ++i){
			const CHARTYPE second = (char)libbase64_characters::toBinary<UCHARTYPE>(GETITEM_BOUNDCHECK(runner + 1, (uint8_t)encodedbounds));
			const CHARTYPE third = (char)libbase64_characters::toBinary<UCHARTYPE>(GETITEM_BOUNDCHECK(runner + 2, (uint8_t)encodedbounds));
			result += (char)(libbase64_characters::toBinary<UCHARTYPE>(GETITEM_BOUNDCHECK(runner, (uint8_t)encodedbounds)) << 2) + ((second & 0x30) >> 4);
			result += (char)((second & 0xf) << 4) + ((third & 0x3c) >> 2);
			result += (char)((third & 0x3) << 6) + libbase64_characters::toBinary<UCHARTYPE>(GETITEM_BOUNDCHECK(runner + 3, (uint8_t)encodedbounds));
			runner += 4;
		}
		
		//now do the ones that might have padding, the first two characters can not be padding, so do them quickly
		const CHARTYPE second = (char)libbase64_characters::toBinary<UCHARTYPE>(GETITEM_BOUNDCHECK(runner + 1, (uint8_t)encodedbounds));
		result += (char)(libbase64_characters::toBinary<UCHARTYPE>(GETITEM_BOUNDCHECK(runner + 0, (uint8_t)encodedbounds)) << 2) + ((second & 0x30) >> 4);
		runner += 2;
		if ((runner != end) && (*runner != (CHARTYPE)'=')){  //not two = pads
			LIBBASE64CODECOVERAGEBRANCH;
			const CHARTYPE third = (char)libbase64_characters::toBinary<UCHARTYPE>(GETITEM_BOUNDCHECK(runner, (uint8_t)encodedbounds));
			result += (char)((second & 0xf) << 4) + ((third & 0x3c) >> 2);
			++runner;
			if ((runner != end) && (*runner != (CHARTYPE)'=')){  //no padding
				LIBBASE64CODECOVERAGEBRANCH;
				result += (char)(((third & 0x3) << 6) + (uint8_t)libbase64_characters::toBinary<UCHARTYPE>(GETITEM_BOUNDCHECK(runner, (uint8_t)encodedbounds)));
			} else {
				LIBBASE64CODECOVERAGEBRANCH;
			}
		} else {
