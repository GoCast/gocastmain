...
public:
    ...
    inline void erase(JSONNode ** & position) json_nothrow {
	   ...
	   std::memmove(position, position + 1, (mysize-- - uint32_t(position - array) - 1) * sizeof(JSONNode *));
    ...
    inline void erase(JSONNode ** position, json_index_t number, JSONNode ** & starter) json_nothrow {
	   ...
	   std::memmove(position + 1, position, (mysize++ - uint32_t(position - array)) * sizeof(JSONNode *));
	   *position = item;
    }
    ...
    void insert(JSONNode ** & position, JSONNode ** items, json_index_t num) json_nothrow {
	   ...
	   const size_t ptrs = size_t(((JSONNode **)(array + mysize)) - position);
