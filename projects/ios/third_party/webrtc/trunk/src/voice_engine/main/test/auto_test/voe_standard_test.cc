...
namespace voetest {
    ...
    int GetDocumentsDir(char* buf, int bufLen)
    {
        ...
        assert(bufLen >= strlen([basePath UTF8String]));

        strcpy(buf, [basePath UTF8String]);
        return 0;
    }

    int GetResourceDir(char* buf, int bufLen)
    {
        assert(buf && bufLen);
    ...
    {
        return (const char*)GetFilename((char*)filename);
    }
    ...
    int GetResource(char* resource, char* dest, int destLen)
    {
        ...
        GetDocumentsDir(path, 1024);
        assert(destLen > strlen(path) + strlen(resource) + 1);
        sprintf(dest, "%s/%s", path, resource);
        return 0;
    }
    ...
    char* GetResource(char* resource)
    {
        ...
        GetResource(resource, result, 1024);
        return strdup(result);
    }

    const char* GetResource(const char* resource)
    {
        return (const char*)GetResource((char*)resource);
    }
...
bool SubAPIManager::GetExtendedMenuSelection(ExtendedSelection& sel, int optionNumberTwo)
{
    printf("------------------------------------------------\n");
...
int runAutoTest(TestType testType, ExtendedSelection extendedSel, int optionNumberTwo)
{
    ...
    else if (testType == Extended)
    {
        ...
        while (extendedSel != XSEL_None)
        {
            ...
            apiMgr.GetExtendedMenuSelection(extendedSel, optionNumberTwo);
        }  // while (extendedSel != XSEL_None)
    }
...
extern "C"
{
int RunInManualMode(int selection, int optionNumberTwo)
{
...
//    dummy = scanf("%d", &selection);
    ...
    switch (selection)
    {
      ...
      case 2:
        testType = Extended;
        while (!apiMgr.GetExtendedMenuSelection(extendedSel, optionNumberTwo))
          continue;
        break;
      case 3:
        testType = Stress;
    ...
    return runAutoTest(testType, extendedSel, optionNumberTwo);
}
};
