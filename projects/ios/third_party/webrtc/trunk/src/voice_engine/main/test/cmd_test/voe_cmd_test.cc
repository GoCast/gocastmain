...
void RunTest(std::string out_path, char* remoteIP, int remotePort, int localPort, int codecChoice, int sendListenPlayoutOption);
#else
...
#if defined(MAC_IPHONE)
int runVOECMDIOSTest(char* remoteIP, int remotePort, int localPort, int codecChoice, int sendListenPlayoutOption)
#else
int main()
#endif
{
  ...
  const std::string out_path = "/sdcard/";
    ...
    {
        char buf[1024];
        GetDocumentsDir(buf, 1024);
        out_path = (const char*)buf;
        out_path += "/";
    }
#else
...
#if defined(MAC_IPHONE)
    RunTest(out_path, remoteIP, remotePort, localPort, codecChoice, sendListenPlayoutOption);
#else
    RunTest(out_path);
#endif
